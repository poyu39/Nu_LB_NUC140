...
void SetPACA(unsigned char PA, unsigned char CA)
{
    // Set PA
		...
		SPI_SET_SS0_LOW(SPI_PORT[eDRVSPI_PORT3]);
    SPI_PORT[eDRVSPI_PORT3]->TX[0] = 0xB0 | PA;
    SPI_PORT[eDRVSPI_PORT3]->CNTRL |= SPI_CNTRL_GO_BUSY_Msk;
    while((SPI_PORT[eDRVSPI_PORT3]->CNTRL &¡@SPI_CNTRL_GO_BUSY_Msk) == SPI_CNTRL_GO_BUSY_Msk);       //check data out?
    // Set CA MSB
    ...
    SPI_PORT[eDRVSPI_PORT3]->CNTRL |= SPI_CNTRL_GO_BUSY_Msk;
    while((SPI_PORT[eDRVSPI_PORT3]->CNTRL &¡@SPI_CNTRL_GO_BUSY_Msk) == SPI_CNTRL_GO_BUSY_Msk);       //check data out?
		SPI_SET_SS0_HIGH(SPI_PORT[eDRVSPI_PORT3]);
