...
void send_player1_paddle_pos(void) {
    p_send_int(player1.x);
}
...
void start_countdown() {
    ...
    player2.x = 63;
    if (DEBUG) {
        p_send_int(debug_ball.dx);
        while (!RX_FLAG);
        ball1.dx = get_rx_int();
        p_send_int(debug_ball.dy);
        while (!RX_FLAG);
        ball1.dy = get_rx_int();
    } else {
        if (is_master) {
...
void update_player_paddle(void) {
    uint16_t temp = mapping(get_turn_button(), 0, 4095, 8, 127 - 8);
    if (player1.x != temp) {
        player1.x = temp;
        send_player1_paddle_pos();
    }
    if (RX_FLAG) {
        if (RX_FLAG) {
            player2.x = 127 - get_rx_int();
        }
    }
}
...
void update_ball(void) {
    if (check_hit_ground(&ball1, &player1, &player2)) {
        ...
        reset_ball(&ball1);
        if (DEBUG) {
            debug_ball.dx = ball1.dx;
            debug_ball.dy = ball1.dy;
            ball1.dx = 0;
            ball1.dy = 0;
        }
        check_who_win();
...
int main(void) {
    ...
    init_ball(&debug_ball, 63, 31, 1, 1, 1, ball_bmp);
    ...
    while (TRUE) {
        ...
        case 2:
            // update
            // if(change_ball_speed(&ball1)) p_send_int(ball1.speed);
